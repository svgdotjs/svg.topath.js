...
describe('attribute transfer', () => {
  it('transfers fill, stroke, opacity and transform', () => {
    const rect = draw
      .rect(200, 100)
  ...
  it('transfers presentation attributes derived from those names', () => {
    const rect = draw
      .rect(200, 100)
      .fill({ color: '#f06', opacity: 0.5 })
      .stroke({ color: '#ff6', opacity: 0.7, width: 5, dasharray: '4 2' })

    const path = rect.toPath()

    assert.strictEqual(path.attr('stroke-width'), 5)
    assert.strictEqual(path.attr('stroke-opacity'), 0.7)
    assert.strictEqual(path.attr('stroke-dasharray'), '4 2')
    assert.strictEqual(path.attr('fill-opacity'), 0.5)
  })

  it('does not copy geometry or unrelated attributes onto the path', () => {
    const rect = draw
      .rect(200, 100)
      .move(10, 20)
      .attr({ rx: 5, 'data-x': 1, 'data-fill': 'red' })

    const path = rect.toPath()

    assert.strictEqual(path.node.getAttribute('width'), null)
    assert.strictEqual(path.node.getAttribute('height'), null)
    assert.strictEqual(path.node.getAttribute('x'), null)
    assert.strictEqual(path.node.getAttribute('y'), null)
    assert.strictEqual(path.node.getAttribute('rx'), null)
    assert.strictEqual(path.node.getAttribute('data-x'), null)
    assert.strictEqual(path.node.getAttribute('data-fill'), null)
  })
})
